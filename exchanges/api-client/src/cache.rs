...
where
    ...
    ST: Storage<Output = Bytes> + Send + Sync + 'static,
    Buffer<S, Request>: Service<Request, Response = S::Response>,
    <Buffer<S, Request> as Service<Request>>::Error: From<anyhow::Error> + Send,
    <Buffer<S, Request> as Service<Request>>::Future: Send,
{
    type Response = ST::Output;

    type Error = <Buffer<S, Request> as Service<Request>>::Error;
    type Future = Pin<Box<dyn Future<Output = Result<Self::Response, Self::Error>> + Send>>;
